...
public class UCSBDiningCommonsMenuItemIT {
  ...
  UCSBDiningCommonsMenuItemRepository
      ucsbDiningCommonsMenuItemRepository; // Not sure if it's lowercase
  ...
  public void an_admin_user_can_post_a_new_ucsbdiningcommonsmenuitem() throws Exception {
    ...
    MvcResult response =
        mockMvc
            .perform(
                post("/api/ucsbdiningcommonsmenuitem/post?diningcommonscode=ortega&name=Chicken&station=Entrees")
                    .with(csrf()))
            .andExpect(status().isOk())
            .andReturn();
